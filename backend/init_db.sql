...
CREATE TABLE IF NOT EXISTS DonHang (
    ...
    ghichu TEXT,
    tongtien REAL,
    trangthai TEXT DEFAULT 'Đã đặt',
    lydo_huy TEXT
);

-- Migration: thêm cột nếu DB đã tồn tại (SQLite không có IF NOT EXISTS cho ADD COLUMN)
-- Dùng INSERT OR IGNORE vào sqlite_master để kiểm tra trước
-- Script migration riêng sẽ được gọi từ database.py
